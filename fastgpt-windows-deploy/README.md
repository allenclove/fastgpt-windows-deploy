...
如果 `minio.exe` 只有几百字节，说明是 Git LFS 指针文件而非真实二进制。解决方法：
...
如果 `pnpm build:sdks` 报错 `process.getBuiltinModule is not a function`，说明 Node.js 版本过低。SDK 构建需要 Node.js 22+，但 FastGPT 运行只需 20.x。

解决方法：
1. 安装 Node.js 22+ 用于构建
2. 或使用 `pnpm install --ignore-scripts` 跳过构建，直接运行
...
- `MongoDB keyFile 缺失`：start.bat 自动生成 keyFile
- `MinIO LFS 指针`：verify.bat 自动检测 LFS 指针文件
- `SDK 构建需 Node.js 22+`：setup.bat 提示并尝试继续