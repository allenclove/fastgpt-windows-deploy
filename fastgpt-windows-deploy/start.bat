...
:: =============================================================
:: 5. 启动 Mock 插件服务 (FastGPT 启动必须)
:: =============================================================
echo [5/7] 启动 Mock 插件服务...

:: 检查端口 4004 是否未被排除
netsh interface ipv4 show excludedportrange protocol=tcp 2>&1 | findstr "4004" >nul
if %errorlevel% equ 0 (
    echo   [WARNING] 端口 4004 在 Windows 保留范围，尝试使用 4104
    set "PLUGIN_PORT=4104"
) else (
...
:: 更新 .env 中的插件端口
if !PLUGIN_PORT! neq 4004 (
    powershell -Command "(Get-Content '%CONFIG_DIR%\.env') -replace 'PLUGIN_BASE_URL=http://localhost:4004','PLUGIN_BASE_URL=http://localhost:!PLUGIN_PORT!' | Set-Content '%CONFIG_DIR%\.env'"
)
echo.
...
:: 检查端口 3000 是否被 Windows 保留
set "APP_PORT=3000"
netsh interface ipv4 show excludedportrange protocol=tcp 2>&1 | findstr "3000" >nul
if %errorlevel% equ 0 (
    echo   [INFO] 端口 3000 被 Windows 保留，改用 4000
    set "APP_PORT=4000"
)
