...
export function createVitestStorageMock(params) {
    ...
    const checkObjectExists = vi.fn(async ({ key }) => {
        return { bucket: bucketName, key, exists: objects.has(key) };
    });
    const uploadObject = vi.fn(async (p) => {
    ...
    const downloadObject = vi.fn(async (p) => {
    ...
    const deleteObject = vi.fn(async (p) => {
        objects.delete(p.key);
        return { bucket: bucketName, key: p.key };
    });
    const deleteObjectsByMultiKeys = vi.fn(async (p) => {
        for (const key of p.keys)
            objects.delete(key);
        return { bucket: bucketName, keys: p.keys };
    });
    const deleteObjectsByPrefix = vi.fn(async (p) => {
        if (!p.prefix) {
    ...
    const generatePresignedPutUrl = vi.fn(async (p) => {
    ...
    const generatePresignedGetUrl = vi.fn(async (p) => {
    ...
    const generatePublicGetUrl = vi.fn(({ key }) => {
    ...
    const listObjects = vi.fn(async (p) => {
        const keys = Array.from(objects.keys()).filter((k) => p.prefix ? k.startsWith(p.prefix) : true);
        keys.sort();
        return { bucket: bucketName, keys };
    });
    const copyObjectInSelfBucket = vi.fn(async (p) => {
    ...
    const getObjectMetadata = vi.fn(async (p) => {
        const obj = objects.get(p.key);
        if (!obj) {
            throw new Error(`Object not found: ${p.key}`);
        }
...
//# sourceMappingURL=vitestMock.js.map