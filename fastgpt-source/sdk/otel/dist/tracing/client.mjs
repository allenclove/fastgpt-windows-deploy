...
function hasOtlpEndpoint(config) {
    if (config?.url)
        return true;
    if (getEnvironmentVariable('OTEL_EXPORTER_OTLP_TRACES_ENDPOINT'))
        return true;
    if (getEnvironmentVariable('OTEL_EXPORTER_OTLP_ENDPOINT'))
        return true;
    return false;
}
function normalizeOtlpTracesUrl(url) {
...
function resolveOtlpTracesUrl(config) {
    if (config?.url)
        return config.url;
...
function normalizeTracingOptions(options) {
    if (options === false) {
        return {
            enabled: false,
            sampleRatio: 1
...
export async function configureTracing(options = {}) {
    ...
    configurePromise = (async () => {
        ...
        const resource = defaultResource().merge(resourceFromAttributes({
            [ATTR_SERVICE_NAME]: tracingOptions.serviceName ??
